...
interface McpMethodErrorStatCardProps {
  readonly timeRange: GlobalTimeRange;
  readonly refreshKey?: number | undefined;
}

export function McpMethodErrorStatCard({ timeRange, refreshKey }: McpMethodErrorStatCardProps): React.JSX.Element {
  ...
  const {
    ...
    queryKey: ['mcp-stats-errors', apiRange, refreshKey],
    queryFn: async () => {
      const res = await getMcpStatsErrors({ range: apiRange as McpStatsRange, dimension: 'global' });
      if (!res.ok) {
    ...
    <Card className="flex h-full flex-col">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center text-sm font-medium text-muted-foreground">
          ...
          {t('dashboard.mcp.top_error_methods', 'Top Error Methods')}
        </CardTitle>
      </CardHeader>
      <CardContent className="relative flex-1 pt-2 pb-6">
        {((): ReactNode => {
          if (error !== null) {
            return (
              <div className="flex h-[150px] items-center justify-center text-sm text-destructive">{String(error)}</div>
            );
          }
          if (isLoadingInitial) {
            return (
          ...
          return (
            <div className={`space-y-4 transition-opacity duration-300 ${loading ? 'opacity-50' : 'opacity-100'}`}>
              ...
              {list.map((item, _index) => {
                const maxCount = Math.max(...list.map((d) => d.count));
                const widthRatio = Math.max((item.count / maxCount) * 100, 2);
                return (
                  <div key={item.method} className="flex flex-col space-y-1.5">
                    <div className="flex items-center justify-between text-sm">
                      <span className="truncate pr-4 font-medium" title={item.method}>
                        {item.method}
                      </span>
                      <span className="shrink-0 font-medium text-destructive">{item.count}</span>
                    </div>
