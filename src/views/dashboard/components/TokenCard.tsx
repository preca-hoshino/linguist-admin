...
export function TokenCard({ today, loading }: TokenCardProps): React.JSX.Element {
    ...
    return (
      <Card className="gap-4 py-4">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-0">
          <CardTitle className="text-sm font-medium">{t('dashboard.todayTokens')}</CardTitle>
  ...
  const cached = today?.today_cached_tokens ?? 0;
