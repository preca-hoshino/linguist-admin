...
// ── 内部组件：请求与响应 Tab
function ToolRequestTab({ toolCall, isJsonArgs, parsedArgs, t }: {
  ...
  return (
    <TabsContent value="request" className="h-full m-0 flex flex-col min-h-0">
      {toolCall == null ? (
        <div className="flex h-full items-center justify-center text-sm text-muted-foreground border border-border/50 rounded-md bg-background">{t('modelsPage.logs.detail.noRequestParams', '暂无请求参数')}</div>
      ) : (
        <div className='overflow-y-auto flex-1 min-h-0 min-w-0 scrollbar-thin'>
          {isJsonArgs && parsedArgs != null ? (
                    ...
                    <table className='w-full text-left border-collapse min-w-full'>
                      <thead>
                        <tr className='bg-muted/40 dark:bg-muted/20 border-b border-border/40 text-[12px] font-medium text-muted-foreground'>
                          <th className='px-6 py-4 font-medium whitespace-nowrap w-fit min-w-[160px] sm:min-w-[200px] border-r border-border/40 tracking-wider'>{t('modelsPage.logs.detail.field', '字段 (Field)')}</th>
                          <th className='px-6 py-4 font-medium w-full tracking-wider'>{t('modelsPage.logs.detail.value', '值 (Value)')}</th>
                        </tr>
                      </thead>
                            ...
                            <td className='px-6 py-4 sm:py-5 align-top font-mono text-[13px] text-foreground break-all sm:break-words'>
                              {((): React.ReactNode => {
                                if (val === undefined || val === null) {
                                  return <span className='italic opacity-50'>{val === null ? 'null' : 'undefined'}</span>;
                                }
                                if (val === '') {
                                  return <span className='italic opacity-50'>{t('common.emptyString', '[空字符串]')}</span>;
                                }
                                ...
                                return <span className='whitespace-pre-wrap leading-relaxed break-words'>{typeof val === 'string' ? val : String(val as string | number | boolean)}</span>;
                              })()}
                            </td>
                          </tr>
                        ))}
            ...
            <pre className='whitespace-pre-wrap break-all font-mono text-[13px] text-foreground leading-relaxed bg-background border border-border/50 rounded-md p-5 min-h-full'>
              {typeof toolCall.function.arguments === 'string' ? toolCall.function.arguments : JSON.stringify(toolCall.function.arguments, null, 2)}
            </pre>
          )}
        </div>
      )}
...
function ToolResponseTab({ toolResponse, responseType, parsedJson, resolvedTheme, t }: {
  ...
  return (
    <TabsContent value="response" className="flex-1 min-h-0 m-0 overflow-y-auto scrollbar-thin">
      {toolResponse == null || toolResponse === '' ? (
        <div className="flex h-full items-center justify-center text-sm text-muted-foreground/50 italic">{t('modelsPage.logs.detail.noToolResult', '暂无工具返回结果')}</div>
      ) : (
        <div className='flex flex-col gap-3 min-h-full'>
          ...
          <div className='w-full rounded-md border border-border/40 bg-background/60 px-6 py-4'>
            {responseType === 'json' && parsedJson !== undefined ? (
              ...
              <MarkdownViewer content={toolResponse} enableXmlHighlight={responseType === 'xml'} className='text-[13px] leading-relaxed' />
            )}
          </div>
        </div>
      )}
...
export function ToolInteractionDialog({
  ...
  const { parsedArgs, isJsonArgs } = useMemo(() => {
    if (toolCall == null || toolCall.function.arguments === '') return { parsedArgs: null, isJsonArgs: false }
  ...
  const { responseType, parsedJson } = useMemo(() => {
    if (toolResponse == null || toolResponse === '') return { responseType: 'markdown' as ResponseType, parsedJson: undefined }
    ...
    <Dialog>
      <DialogContent showCloseButton={false} className='flex flex-col h-[85vh] max-h-[850px] min-h-[540px] w-[95vw] sm:max-w-[960px] overflow-hidden p-0 gap-0'>
        <Tabs defaultValue={defaultTab} className="flex flex-col flex-1 min-h-0 w-full overflow-hidden">
          <DialogHeader className='flex flex-row items-center justify-between shrink-0 border-b px-8 py-5 bg-background h-[72px] relative'>
            <div className='flex items-center gap-3 h-full z-10 w-full min-w-0 pr-8'>
               <Wrench className='h-5 w-5 text-foreground shrink-0 mt-0.5' />
               <div className='flex flex-col gap-1 text-left justify-center h-full min-w-0 flex-1'>
                 <DialogTitle className='leading-tight flex items-center break-all sm:break-normal truncate sm:whitespace-normal sm:line-clamp-2 text-[15px] max-h-full'>
          ...
          <div className='flex-1 min-h-0 w-full min-w-0 px-8 py-6 bg-muted/10 overflow-hidden flex flex-col gap-5'>
            <div className="flex w-full shrink-0 justify-center">
              <TabsList className="flex">
                <TabsTrigger value="request" className="text-[12px] px-4 w-32">{t('modelsPage.logs.detail.toolRequest', '请求参数')}</TabsTrigger>
                <TabsTrigger value="response" className="text-[12px] px-4 w-32">{t('modelsPage.logs.detail.toolResponse', '执行结果')}</TabsTrigger>
              </TabsList>
            </div>

            <ToolRequestTab toolCall={toolCall} isJsonArgs={isJsonArgs} parsedArgs={parsedArgs} t={t} />
            <ToolResponseTab toolResponse={toolResponse} responseType={responseType} parsedJson={parsedJson} resolvedTheme={resolvedTheme} t={t} />
          </div>
        </Tabs>
      </DialogContent>
    </Dialog>
...
// ── 工具交互弹窗的触发元素（在气泡中使用）
export const ToolInteractionTrigger = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & {
  readonly toolName: string
  readonly callId?: string | undefined
  readonly defaultTab: 'request' | 'response'
  ...
  return (
    <div
      ...
      role='button'
      tabIndex={0}
      className={cn('flex w-full items-center justify-between pt-1 pb-0 opacity-80 hover:opacity-100 transition-opacity cursor-pointer font-sans text-foreground mb-1 last:-mb-1', className)}
      {...props}
    >
      <div className='flex items-center gap-3 w-full min-w-0 pl-1 my-0.5'>
        ...
        <div className='flex flex-col text-left justify-center min-w-0 w-full gap-1'>
          ...
          {displayId !== 'Unknown ID' && (
            <div onClick={(e) => { e.stopPropagation(); }} className='max-w-full flex items-center'>
      ...
      <span className='text-[10px] text-muted-foreground shrink-0 ml-2 opacity-60'>
        {defaultTab === 'request' ? t('modelsPage.logs.detail.viewParams', '查看参数') : t('modelsPage.logs.detail.viewResult', '查看结果')}
      </span>
    </div>
  )
})
