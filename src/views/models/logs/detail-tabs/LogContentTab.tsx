...
function ChatContent({
  ...
  readonly usage?: { prompt_tokens?: number; total_tokens?: number; completion_tokens?: number; cached_tokens?: number; reasoning_tokens?: number } | undefined;
  readonly t: (key: string, fallback: string) => string;
}): React.JSX.Element {
  const chatReq = ctx.request as { messages?: AuditMessage[] } | undefined
  const chatResp = ctx.response as { choices?: Array<{ message?: { content?: string | null, reasoning_content?: string, tool_calls?: AuditToolCall[] } }> } | undefined
  ...
  const allMessages = useMemo(() => {
    ...
    if (
      assistantMsg &&
      (arr.length === 0 || arr.at(-1)?.role !== 'assistant')
    ) {
      const newMsg: Partial<AuditMessage> = { role: 'assistant' }
      if (assistantMsg.content != null && assistantMsg.content !== '') newMsg.content = assistantMsg.content
      if (assistantMsg.reasoning_content != null && assistantMsg.reasoning_content !== '') newMsg.reasoning_content = assistantMsg.reasoning_content
      if (assistantMsg.tool_calls) newMsg.tool_calls = assistantMsg.tool_calls
  ...
  const toolCallMap = useMemo(() => {
    ...
    for (const m of allMessages) {
      if (m.role === 'assistant' && m.tool_calls) {
        for (const tc of m.tool_calls) map.set(tc.id, tc)
      }
    }
    return map
  }, [allMessages])
      ...
      <div className='flex flex-col flex-1 overflow-y-auto p-4 w-full pt-4'>
        {systemItems.length > 0 && (
          <div className='flex flex-col mb-2'>
            {systemItems.map((sys) => <SystemBanner key={sys.id} msg={sys} />)}
          </div>
        )}
        ...
        {dialogItems.length > 0 ? (
          <div className='flex flex-col w-full pb-10'>
            {dialogItems.map((item, idx) => {
              ...
              const isMerged = !!(prev && prev.role === item.role && prev.nameLabel === item.nameLabel && prev.isRight === item.isRight)
              return <ChatBubble key={item.id} item={item} isMerged={isMerged} isFirst={idx === 0} toolCallMap={toolCallMap} toolResponseMap={toolResponseMap} />
            })}
          </div>
        ) : (
...
function EmbeddingContent({
  ...
  const embeddingReq = ctx.request as { input?: Array<{ type?: string; text?: string; url?: string; base64_data?: string }> | string | string[] } | undefined
