...
import {
  Network, Key, Globe, Cpu, Layers, AlertTriangle,
...
const CAP_STYLE: Record<string, string> = {
  vision: 'bg-violet-50 border-violet-300 text-violet-800 dark:bg-violet-500/10 dark:text-violet-300 dark:border-violet-500/30',
  tools: 'bg-amber-50 border-amber-300 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300 dark:border-amber-500/30',
  thinking: 'bg-emerald-50 border-emerald-300 text-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/30',
  ...
  multimodal: 'bg-fuchsia-50 border-fuchsia-300 text-fuchsia-800 dark:bg-fuchsia-500/10 dark:text-fuchsia-300 dark:border-fuchsia-500/30',
}
...
function InfoRow({ label, icon: Icon, children, className }: {
  ...
  return (
    <div className={cn('grid grid-cols-[160px_1fr] items-start gap-3 py-2.5 border-b border-border/50 last:border-0', className)}>
...
function InfoText({ text, mono }: { readonly text?: string | null; readonly mono?: boolean }): React.JSX.Element {
  if (text == null || text === '') return <span className='text-muted-foreground text-sm'>—</span>
  return (
    <span className={cn('text-sm break-all', mono && 'font-mono text-xs')}>
      {text}
    </span>
  )
}
        ...
        <div>
          <InfoRow label={t('modelsPage.logs.detail.status', '状态')} icon={StatusIcon}>
            <Badge
              ...
              className={cn(
                'font-medium',
                log.status === 'completed' && 'border-green-300 text-green-700 bg-green-50/50 dark:border-green-900 dark:text-green-400 dark:bg-green-900/20',
            ...
            >
              {(((): string => {
                if (log.status === 'completed') return t('modelsPage.logs.statusCompleted', '成功');
                if (log.status === 'error') return t('modelsPage.logs.statusError', '失败');
                    ...
                    {route.capabilities.map((cap) => (
                      <Badge
                        ...
                        className={cn(
                          ...
                          CAP_STYLE[cap] != null && CAP_STYLE[cap] !== '' ? CAP_STYLE[cap] : 'bg-muted/50 border-border/40 text-muted-foreground'
                        )}
                      >
                        {cap}
                      </Badge>
          ...
          <InfoRow label={t('modelsPage.logs.detail.stream', '工作模式 (Stream)')}>
            <Badge variant='outline' className={ctx.stream ? 'border-blue-200 text-blue-600 bg-blue-50/50 dark:bg-blue-500/10 dark:border-blue-500/20 dark:text-blue-400' : 'text-muted-foreground'}>
              {ctx.stream
                ? t('modelsPage.logs.stream', '流式工作')
                : t('modelsPage.logs.nonStream', '阻断响应')}
            </Badge>
          </InfoRow>
        ...
        <div>
          <InfoRow label={t('modelsPage.logs.detail.apiKey', '认证令牌 (API Key)')} icon={Key}>
            <span className='text-sm'>
              ...
              {(ctx.apiKeyName == null || ctx.apiKeyName === '') && (ctx.apiKeyPrefix == null || ctx.apiKeyPrefix === '') && <span className='text-muted-foreground opacity-50'>未追踪到明确的 API Key / 或者匿名请求</span>}
            </span>
          </InfoRow>
