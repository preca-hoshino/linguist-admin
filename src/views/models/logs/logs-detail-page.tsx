...
import {
  ChevronLeft, Clock, Trash2, MessageSquare,
  Wrench, Code2, User, RouterIcon, Cloud, ArrowRight, FileText, Activity, CreditCard, Tags
} from 'lucide-react'
...
function mapFormatToProvider(f?: string | null): string | null {
  ...
  if (f === 'openaicompat') return 'openai'
...
function formatUserFormat(f?: string | null): string {
  if (f == null || f === '') return 'unknown'
  if (f === 'openaicompat') return 'openai compat'
  return f
}
...
function LogPageHeader({ log }: { readonly log: RequestLog }): React.JSX.Element {
  ...
  const getStrategyName = (s?: string | null): string => {
    if (s === 'load_balance') return t('modelsPage.virtualModels.strategyLoadBalance', 'Load Balance')
    if (s === 'failover') return t('modelsPage.virtualModels.strategyFailover', 'Failover')
    if (s === 'fallback') return t('modelsPage.virtualModels.strategyFailback', 'Failback')
  ...
  const renderNode = (
    ...
    <div className='flex flex-col items-center gap-2 shrink-0 z-10 w-24'>
      ...
      <div className='flex flex-col items-center text-center'>
        ...
        {desc1 != null && desc1 !== '' && (
          <span className='text-[11px] font-mono text-muted-foreground mt-0.5 truncate max-w-[100px]'>
            {desc1}
          </span>
        )}
        {desc2 != null && desc2 !== '' && (
  ...
  const renderEdge = (label: string, providerKind?: string | null, isError?: boolean): React.JSX.Element => (
    ...
    >
      {/* 水平连线 */}
      <div className={cn('absolute w-full h-[1px] left-0 top-1/2 -translate-y-1/2', isError ? 'bg-destructive/40 border-dashed border-t border-destructive/40 bg-transparent' : 'bg-border')} />
      {/* 小箭头 */}
      ...
      <ProviderBadge 
        ...
        label={label} 
        isError={isError} 
        className="z-10 bg-background shadow-sm" 
      />
    </div>
  )
    ...
    <div className='flex flex-col gap-6 mb-2'>
          ...
          <div>
            ...
            <div className='mt-1.5 flex items-center gap-2'>
              <Badge
                variant='outline'
                className={cn(
                  isCompleted && 'border-green-300 text-green-700 bg-green-50/50 dark:border-green-900 dark:text-green-400 dark:bg-green-900/20',
                  isError && 'border-destructive/40 text-destructive bg-destructive/5',
                  !isCompleted && !isError && 'text-muted-foreground'
                )}
              >
                {(((): string => {
                  if (isCompleted) return t('modelsPage.logs.statusCompleted', '成功');
                  if (isError) return t('modelsPage.logs.statusError', '失败');
      ...
      <div className='flex items-center justify-center px-4 py-8 relative max-w-full overflow-x-auto'>
        ...
        {renderNode(
          ...
          'Linguist',
          null,
          getStrategyName(ctx?.route?.strategy)
        )}

        {/* Linguist -> Provider */}
        {renderEdge(
          ctx?.route?.providerKind ?? 'unknown',
